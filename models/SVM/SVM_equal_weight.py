...
path = 'SVM_imbalance_eq_weight_100'
...
def hyperparameter_tuning(space):
    ...
    loss = 0.5 * (1 - np.mean(recall_test)) + 0.5 * delta
...
for j in range(20):
    ...
    # train_acc = np.zeros(21)
    # test_acc = np.zeros(21)
    # deltas = np.zeros(21)

    if os.path.isfile(f'{new_path}/best_model_recall_loop_{j}.joblib'):
        print(f"{new_path}/best_model_recall_loop_{j}.joblib, exists, skipping to next iteration.")
        continue

    trials = Trials()
    best_i = fmin(hyperparameter_tuning, space, algo=tpe.suggest, max_evals=100, trials=trials)
    print(best_i)
    ...
    dump(best_i, f'{new_path}/best_model_recall_loop_{j}.joblib')
    dump(test, f'{new_path}/test_model_recall_loop_{j}.joblib')
